...
class PrusaSlicer(BaseSlicer):
    def check_identity(self, data: str) -> Optional[Dict[str, str]]:
        aliases = {
            ...
            'FlsunSlicer': r"FlsunSlicer\s(.*)\son"         # FLSUN
        }
    ...
    # Add function to parse multi-color information
    def parse_filament_colour(self) -> Optional[str]:
        return _regex_find_string(
            r";\sfilament_colour\s=\s(.*)", self.footer_data)
    
    # Add function to parse model information
    def parse_printer_model(self) -> Optional[str]:
        return _regex_find_string(
            r";\sprinter_model\s=\s(.*)", self.footer_data)
...
SUPPORTED_DATA = [
    ...
    'chamber_temp',
    'filament_name',
    'filament_colour',   # Add multi-color information
    'printer_model',     # Add model information
    'filament_type',
    'filament_total',
...
def find_utf8_boundary(header_data: bytes) -> str:
    """
    从 header_data 数组中查找 UTF-8 字符的边界，并去掉非边界的内容。

    :param header_data: 输入的字节数组。
    :return: 去掉非边界内容后的完整字节数组。
    """
    # 从数组末尾向前查找 UTF-8 字符的边界
    i = 0
    while i < len(header_data):
        byte = header_data[i]
        if (byte & 0b11000000) != 0b10000000:  # 检查是否是 UTF-8 字符的起始字节
            break
        i += 1

    # 返回从边界开始的完整内容
    return header_data[i:].decode()
...
def get_slicer(file_path: str) -> Tuple[BaseSlicer, Dict[str, str]]:
    ...
    with open(file_path, 'rb') as f:
        ...
        # identify the slicer
        header_data_array = f.read(READ_SIZE)
        header_data = find_utf8_boundary(header_data_array)
        ...
        if size > READ_SIZE * 2:
            ...
            footer_data_array = f.read()
        elif size > READ_SIZE:
            remaining = size - READ_SIZE
            footer_data_array = header_data_array[remaining - READ_SIZE:] + f.read()
        else:
            footer_data_array = header_data_array
        footer_data = find_utf8_boundary(footer_data_array)
